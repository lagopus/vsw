...
func (suite *testTickTestSuite) SetupSuite() {
	suite.ticker = NewTicker()
}

func (suite *testTickTestSuite) TearDownTest() {
	suite.ticker.AllUnregisterTask()
}

func (suite *testTickTestSuite) tickerStart() {
	suite.ticker.Start(&wg)
}

func (suite *testTickTestSuite) tickerStop() {
	suite.ticker.Stop()
	wg.Wait()
}
